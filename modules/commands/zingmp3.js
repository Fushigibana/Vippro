...
module.exports.handleReply = async function({ api, event, handleReply }) {
    ...
    api.sendMessage("⚡️Đang xử lý request của bạn!", event.threadID, (err, info) =>
    ...
    if (fs.statSync(__dirname + `/cache/${handleReply.link[event.body - 1]}.mp3`).size > 26214400) return api.sendMessage('⚡️Không thể gửi file vì dung lượng lớn hơn 25MB.', event.threadID, () => fs.unlinkSync(__dirname + `/cache/${handleReply.link[event.body - 1]}.mp3`), event.messageID);
...
return api.sendMessage(`🔊 Có ${link.length} kết quả trùng với từ khoá tìm kiếm của bạn:\n\n${msg}\n⚡️Hãy reply(phản hồi) chọn một trong những tìm kiếm trên`, event.threadID,(error, info) => global.client.handleReply.push({ name: this.config.name, messageID: info.messageID, author: event.senderID, link }), event.messageID);
}
catch{
    api.sendMessage(`⚡️Không tìm thấy từ khoá khớp với:\n ►►►${keywordSearch}\n⚡️Xin thử lại với kết quả khác!`, event.threadID, event.messageID);
}
}
