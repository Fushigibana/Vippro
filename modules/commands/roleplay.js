...
module.exports.handleEvent = async ({ event, api }) => {
    ...
    if (event.body.indexOf("hug") == 0 || event.body.indexOf("ôm") == 0) {
        for (const id of mention) {
            ...
            api.sendMessage({ body: event.mentions[id] + ", Tôi muốn ôm bạn ❤️", mentions: [{ tag: event.mentions[id], id: id }], attachment: createReadStream(path) }, event.threadID, () => unlinkSync(path), event.messageID);
        }
        return;
    }
    if (event.body.indexOf("kiss") == 0 || event.body.indexOf("hôn") == 0 || event.body.indexOf("hun") == 0) {
        for (const id of mention) {
            ...
            api.sendMessage({ body: event.mentions[id] + ", Tôi muốn hôn bạn ❤️", mentions: [{ tag: event.mentions[id], id: id }], attachment: createReadStream(path) }, event.threadID, () => unlinkSync(path), event.messageID);
        }
        return;
    }
    ...
    if (event.body.indexOf("slap") == 0 || event.body.indexOf("tát") == 0 || event.body.indexOf("vả") == 0) {
        for (const id of mention) {
            ...
            api.sendMessage({ body: event.mentions[id] + ", nhận lấy cái tát của tôi đi", mentions: [{ tag: event.mentions[id], id: id }], attachment: createReadStream(path) }, event.threadID, () => unlinkSync(path), event.messageID);
        }
        return;
    }
    if (event.body.indexOf("poke") == 0 || event.body.indexOf("chọc") == 0 || event.body.indexOf("trêu") == 0) {
        for (const id of mention) {
